...
class MerchantsController extends AppController
{
    ...
    public function add()
    {
        ...
        $merchants      = $this->Merchants->find('list',[
            'keyField'      => 'id',
            'valueField'    => 'name',
        ]);               
        
        $this->set('selCategories'  ,$selCategories);
        $this->set('selMerchants'  ,$selMerchants);
        $this->set('isEdit',FALSE);
        $this->set(compact('merchant', 'affiliateNetworks','tags','categories', 'merchants'));
        $this->set('_serialize', ['merchant','affiliateNetworks','tags','categories', 'merchants']);
    }
    ...
    {      
        ...
        if(count($merchant->merchants)>0){
            foreach($merchant->merchants  as $merchants_list){
                $selMerchants[] = $merchants_list->id;
            }
        }
                
        /*$total_merchants = $this->Merchants->find('all');
        foreach ($total_merchants as $total_merchant) 
        {
           $selMerchants[] = $total_merchant->id;
        }*/

        $this->set('selCategories'  ,$selCategories);
        $this->set('selMerchants'  ,$selMerchants);
        $this->set(compact('merchant', 'affiliateNetworks','tags','categories', 'merchants'));
        $this->set('_serialize', ['merchant','tags','categories','affiliateNetworks', 'merchants']);
